...
// GetGivenReviewsLogic 获取发出的评价列表业务逻辑
func GetGivenReviewsLogic(userID int64, req *review.GetGivenReviewsReq) (*review.GetGivenReviewsResp, error) {
	// 设置默认分页参数
	page := 1
	limit := 10
	if req.PageReq != nil {
	...
	// 获取发出的评价列表
	reviews, err := mysql.GetGivenReviews(nil, userID, reviewType, offset, limit)
	if err != nil {
		utils.Errorf("获取发出的评价列表失败: %v", err)
		return &review.GetGivenReviewsResp{
	...
	// 获取总数
	total, err := mysql.CountGivenReviews(nil, userID, reviewType)
	if err != nil {
		utils.Errorf("获取发出的评价总数失败: %v", err)
		return &review.GetGivenReviewsResp{
			Base: &common.BaseResp{
				Code:      500,
				Message:   "系统错误",
